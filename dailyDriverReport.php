...
<body>
<h1 align='center' style = "font size: 10pt"> Driver Report Daily </h1>
<?php
...
mysqli_select_db($con, "project") or die(mysqli_error());
	...
	{	
		...
		$query_payment_rental = "SELECT paymentDate as date, amount as price FROM payment_rental WHERE driverID = $id AND paymentDate LIKE '%$date%'";
		$query_earning_rental = "SELECT SUM(amount) as total FROM payment_rental WHERE driverID = $id AND paymentDate LIKE '%$date%'";
		
		$query_payment_taxi = "SELECT taxibooking.destination as destination, payment_taxi.amount as price FROM payment_taxi INNER JOIN taxibooking ON payment_taxi.taxibookingID = taxibooking.taxibookingID WHERE payment_taxi.driverID = $id AND payment_taxi.paymentDate LIKE '%$date%'";
